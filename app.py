...
.user {background:#111;padding:10px;border-radius:10px;margin:6px;text-align:right;}
.ai {background:#ffffff22;padding:10px;border-radius:10px;margin:6px;}
...
def local_ai(q):
    q=q.lower()
    if "sleep" in q:
        return "Sleep problems often come from stress or irregular habits. Maintain routine and avoid screens."
    if "anxiety" in q:
        return "Anxiety causes restlessness and worry. Breathing and counseling help."
    if "depress" in q:
        return "Depression includes low mood and energy. Seek support early."
...
# ================= DSS =================
def dss(q):
    if not isinstance(q,str): return "Unknown","Mild","Monitor"
    q=q.lower()

    if "suicide" in q:
        return "Suicidal Risk","Critical","Immediate intervention required"
    if "depress" in q:
        return "Depression","Moderate","Consult psychologist"
    if "anxiety" in q:
        return "Anxiety","Mild","Relaxation advised"
    return "Stress","Mild","Lifestyle care"
